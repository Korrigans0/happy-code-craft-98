...
const CreateWACreatureDialog = ({ onCreated }: CreateWACreatureDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    setLoading(true);
    const { error } = await supabase.from("wa_creatures").insert({
      name: form.name, size: form.size, power_level: form.power_level,
      profile: form.profile, ra: form.ra, description: form.description,
      strength: parseInt(form.strength), dexterity: parseInt(form.dexterity),
      constitution: parseInt(form.constitution), intelligence: parseInt(form.intelligence),
      wisdom: parseInt(form.wisdom), charisma: parseInt(form.charisma),
      author: "Personnalisé", created_by: user.id,
    } as any);
    setLoading(false);
