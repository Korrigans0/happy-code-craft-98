...
const CONDITIONS = [
  ...
  "À terre", "Entravé", "Étourdi", "Inconscient"
];
...
const CampaignCombat = ({ campaignId, isGM }: CampaignCombatProps) => {
  ...
  const { data: partyCharacters = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("characters")
        .select("id, name, hp, max_hp, armor_class")
        .in("id", characterIds);
  ...
  const createEncounterMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase
        .from("combat_encounters")
        .insert({ campaign_id: campaignId, name: "Combat" })
        .select()
        .single();
      if (error) throw error;
      return data;
  ...
  if (!encounter) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-gradient-card p-12">
        ...
        {isGM && (
          <Button variant="gold" onClick={() => createEncounterMutation.mutate()}>
