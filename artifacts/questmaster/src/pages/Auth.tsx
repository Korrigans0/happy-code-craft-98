...
const Auth = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    // Password reset not supported in this version — show info message
    setIsSubmitting(false);
    setResetEmailSent(true);
    toast({ title: "Non disponible", description: "La réinitialisation par email n'est pas disponible dans cette version." });
  };
  ...
  const handleGuestLogin = async () => {
    ...
    if (error) {
      // Try to auto-create guest account
      const { error: signUpError } = await signUp(guestEmail, guestPassword, 'Invité');
      if (!signUpError) {
        // Guest created, now sign in
        await signIn(guestEmail, guestPassword);
      } else {
        toast({ title: "Connexion invité indisponible", description: "Une erreur est survenue.", variant: "destructive" });
      }
    }
